...
## 2026-01-05 - [Secure LocalStorage Access]
**Vulnerability:** Uncaught exceptions when accessing `localStorage` in strict privacy modes (e.g., cookies disabled) can cause application crashes (Denial of Service).
**Learning:** `localStorage` is not guaranteed to be available. Direct access without error handling violates the "Fail Securely" principle, as a single failure can halt execution of unrelated scripts.
**Prevention:** Implemented a `safeStorage` wrapper in `utils.js` that catches `SecurityError` and other exceptions, ensuring the application degrades gracefully when storage is unavailable.
